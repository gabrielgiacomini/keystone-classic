...
**Some tools might be disabled to save context. Ask to enable if needed.**

| Category                | Tools                                     | Use Case                                       |
| ----------------------- | ----------------------------------------- | ---------------------------------------------- |
| **MongoDB Admin**       | `mongodb-mcp-server` (~20 tools)          | Database manipulation, schema changes          |
| **Screenshot Analysis** | `zai-mcp-server` (7 tools)                | UI mockups, error screenshots, text extraction |
| **Browser Automation**  | `playwright`, `puppeteer`                 | Browser testing, scraping, E2E tests           |
| **Web Scraping**        | `firecrawl_map`, `firecrawl_scrape`       | Advanced scraping, structured data             |
| **Web Search**          | `websearch_exa`, `firecrawl_search`       | Real-time web search                           |
| **Documentation**       | `context7`, `zread`, `zai-mcp-server`     | Official docs for libraries                    |
| **Web Reading**         | `web-reader`, `zread`, `firecrawl_scrape` | Read web pages, GitHub exploration             |
| **File Operations**     | read, write, edit, glob, grep, bash       | File system operations                         |
| **LSP Tools**           | Language Server Protocol tools            | When installed                                 |

## SPECIAL AGENTS

- `@researcher` - High-intensity research (20-50 scrapes, comprehensive briefs)

### Policy

**Suggest enabling tools when** they're the best solution or significantly faster.

**STOP if** a task would benefit significantly from a disabled tool.
