import { expect } from 'chai';
import DateArrayType from '../DateArrayType.mjs';
import dayjs from 'dayjs';
import customParseFormat from 'dayjs/plugin/customParseFormat.js';
dayjs.extend(customParseFormat);

/** Minimal interface for a single DateArray field instance. */
interface DateArrayField {
	validateInput(data: Record<string, unknown>, callback: (result: boolean) => void): void;
	validateRequiredInput(item: DateArrayTestItem, data: Record<string, unknown>, callback: (result: boolean) => void): void;
	updateItem(item: DateArrayTestItem, data: Record<string, unknown>, callback: () => void): void;
	addFilterToQuery(filter: Record<string, unknown>): Record<string, unknown>;
	inputIsValid(data: Record<string, unknown>, required?: boolean, item?: DateArrayTestItem): boolean;
}

/** Named fields registered on the List by initList. */
interface DateArrayTestFields {
	datearr: DateArrayField;
	'nested.datearr': DateArrayField;
}

/** Minimal document shape produced by `new List.model(...)` in these tests. */
interface DateArrayTestItem {
	datearr: Date[] | string[] | undefined;
	nested: { datearr: Date[] | undefined };
	get(path: string): unknown;
	set(path: string, value: unknown): void;
}

/** Minimal shape of the Keystone List passed to initList / testFieldType. */
interface DateArrayTestList {
	add(fields: Record<string, unknown>): void;
	model: new (data?: Record<string, unknown>) => DateArrayTestItem;
	fields: DateArrayTestFields;
}

export const initList = function (List: DateArrayTestList) {
	List.add({
		datearr: { type: DateArrayType },
		nested: {
			datearr: { type: DateArrayType },
		},
	});
};

export const testFieldType = function (List: DateArrayTestList) {
	describe('invalid options', function () {
		it('should throw when format is not a string', function (done) {
			try {
				List.add({
					invalidFormatOption: { type: DateArrayType, format: /aregexp/ },
				});
			} catch (err: unknown) {
				expect((err as Error).message).to.eql('FieldType.DateArray: options.format must be a string.');
				done();
			}
		});
	});

	it('should default to an empty array', function () {
		const testItem = new List.model();
		expect(testItem.get('datearr')).to.eql([]);
	});

	describe('validateInput', function () {
		it('should validate top level fields', function (done) {
			List.fields.datearr!.validateInput({
				datearr: ['2015-03-03', '2015-03-04'],
			}, function (result: boolean) {
				expect(result).to.be.true;
				done();
			});
		});

		it('should validate nested fields', function (done) {
			List.fields['nested.datearr']!.validateInput({
				nested: {
					datearr: ['2015-03-03', '2015-03-04'],
				},
			}, function (result: boolean) {
				expect(result).to.be.true;
				done();
			});
		});

		it('should validate nested fields with flat paths', function (done) {
			List.fields['nested.datearr']!.validateInput({
				'nested.datearr': ['2015-03-03', '2015-03-04'],
			}, function (result: boolean) {
				expect(result).to.be.true;
				done();
			});
		});

		it('should validate an epoch number', function (done) {
			List.fields.datearr!.validateInput({
				datearr: [1458162309111],
			}, function (result: boolean) {
				expect(result).to.be.true;
				done();
			});
		});

		// A single string will be coerced to an array, so we let it pass
		it('should validate a date string', function (done) {
			List.fields.datearr!.validateInput({
				datearr: '2015-03-03',
			}, function (result: boolean) {
				expect(result).to.be.true;
				done();
			});
		});

		// A single string will be coerced to an array, so we let it pass
		it('should validate an epoch number without an array', function (done) {
			List.fields.datearr!.validateInput({
				datearr: 1458162309111,
			}, function (result: boolean) {
				expect(result).to.be.true;
				done();
			});
		});

		// An empty array clears the value, so we let it pass
		it('should validate an empty array', function (done) {
			List.fields.datearr!.validateInput({
				datearr: [],
			}, function (result: boolean) {
				expect(result).to.be.true;
				done();
			});
		});

		// A blank string clears the value, so we let it pass
		it('should validate a blank string', function (done) {
			List.fields.datearr!.validateInput({
				datearr: '',
			}, function (result: boolean) {
				expect(result).to.be.true;
				done();
			});
		});

		// null clears the value, so we let it pass
		it('should validate null', function (done) {
			List.fields.datearr!.validateInput({
				datearr: null,
			}, function (result: boolean) {
				expect(result).to.be.true;
				done();
			});
		});

		// undefined doesn't change anything, so we let it pass
		it('should validate undefined', function (done) {
			List.fields.datearr!.validateInput({
				datearr: undefined,
			}, function (result: boolean) {
				expect(result).to.be.true;
				done();
			});
		});

		it('should invalidate false', function (done) {
			List.fields.datearr!.validateInput({
				datearr: false,
			}, function (result: boolean) {
				expect(result).to.be.false;
				done();
			});
		});

		it('should invalidate true', function (done) {
			List.fields.datearr!.validateInput({
				datearr: true,
			}, function (result: boolean) {
				expect(result).to.be.false;
				done();
			});
		});

		it('should invalidate a random string', function (done) {
			List.fields.datearr!.validateInput({
				datearr: 'aaa',
			}, function (result: boolean) {
				expect(result).to.be.false;
				done();
			});
		});

		it('should invalidate an array with a random string', function (done) {
			List.fields.datearr!.validateInput({
				datearr: ['aaa'],
			}, function (result: boolean) {
				expect(result).to.be.false;
				done();
			});
		});

		it('should invalidate an array with a string somewhere', function (done) {
			List.fields.datearr!.validateInput({
				datearr: ['2015-03-03', '2015-03-04', 'aaa'],
			}, function (result: boolean) {
				expect(result).to.be.false;
				done();
			});
		});
	});

	describe('validateRequiredInput', function () {
		it('should validate an array of dates', function (done) {
			const testItem = new List.model();
			List.fields.datearr!.validateRequiredInput(testItem, {
				datearr: ['2015-01-01', '2015-01-02'],
			}, function (result: boolean) {
				expect(result).to.be.true;
				done();
			});
		});

		it('should validate a date', function (done) {
			const testItem = new List.model();
			List.fields.datearr!.validateRequiredInput(testItem, {
				datearr: '2015-01-01',
			}, function (result: boolean) {
				expect(result).to.be.true;
				done();
			});
		});

		it('should validate a nested array of dates', function (done) {
			const testItem = new List.model();
			List.fields['nested.datearr']!.validateRequiredInput(testItem, {
				nested: {
					datearr: ['2015-01-01', '2015-01-02'],
				},
			}, function (result: boolean) {
				expect(result).to.be.true;
				done();
			});
		});

		it('should validate a nested array of numbers with a flat paths', function (done) {
			List.fields.datearr!.validateInput({
				'nested.datearr': ['2015-01-01', '2015-01-02'],
			}, function (result: boolean) {
				expect(result).to.be.true;
				done();
			});
		});

		it('should invalidate an empty string', function (done) {
			const testItem = new List.model();
			List.fields.datearr!.validateRequiredInput(testItem, {
				datearr: '',
			}, function (result: boolean) {
				expect(result).to.be.false;
				done();
			});
		});

		it('should invalidate undefined', function (done) {
			const testItem = new List.model();
			List.fields.datearr!.validateRequiredInput(testItem, {
				datearr: undefined,
			}, function (result: boolean) {
				expect(result).to.be.false;
				done();
			});
		});

		it('should validate undefined if a value exists', function (done) {
			const testItem = new List.model({
				datearr: ['2015-01-01'],
			});
			List.fields.datearr!.validateRequiredInput(testItem, {
				datearr: undefined,
			}, function (result: boolean) {
				expect(result).to.be.true;
				done();
			});
		});

		it('should invalidate null', function (done) {
			const testItem = new List.model();
			List.fields.datearr!.validateRequiredInput(testItem, {
				datearr: null,
			}, function (result: boolean) {
				expect(result).to.be.false;
				done();
			});
		});

		it('should invalidate an array with an empty string', function (done) {
			const testItem = new List.model();
			List.fields.datearr!.validateRequiredInput(testItem, {
				datearr: [''],
			}, function (result: boolean) {
				expect(result).to.be.false;
				done();
			});
		});

		it('should invalidate an array with empty strings', function (done) {
			const testItem = new List.model();
			List.fields.datearr!.validateRequiredInput(testItem, {
				datearr: ['2015-01-01', '', '2015-01-02'],
			}, function (result: boolean) {
				expect(result).to.be.false;
				done();
			});
		});
	});

	describe('updateItem', function () {
		it('should update top level fields', function (done) {
			const testItem = new List.model();
			List.fields.datearr!.updateItem(testItem, {
				datearr: ['2015-01-01', '2015-01-02', '2015-01-03', '2015-01-04'],
			}, function () {
				expect(testItem.datearr).to.eql([new Date('2015-01-01'), new Date('2015-01-02'), new Date('2015-01-03'), new Date('2015-01-04')]);
				done();
			});
		});

		it('should update nested fields', function (done) {
			const testItem = new List.model();
			List.fields['nested.datearr']!.updateItem(testItem, {
				nested: {
					datearr: ['2015-01-01', '2015-01-02', '2015-01-03', '2015-01-04'],
				},
			}, function () {
				expect((testItem.nested as Record<string, unknown>).datearr).to.eql([new Date('2015-01-01'), new Date('2015-01-02'), new Date('2015-01-03'), new Date('2015-01-04')]);
				done();
			});
		});

		it('should update nested fields with flat paths', function (done) {
			const testItem = new List.model();
			List.fields['nested.datearr']!.updateItem(testItem, {
				'nested.datearr': ['2015-01-01', '2015-01-02', '2015-01-03', '2015-01-04'],
			}, function () {
				expect((testItem.nested as Record<string, unknown>).datearr).to.eql([new Date('2015-01-01'), new Date('2015-01-02'), new Date('2015-01-03'), new Date('2015-01-04')]);
				(testItem.nested as Record<string, unknown>).datearr = undefined;
				done();
			});
		});

		it('should update empty arrays', function (done) {
			const testItem = new List.model();
			List.fields.datearr!.updateItem(testItem, {
				datearr: [],
			}, function () {
				expect(testItem.datearr).to.eql([]);
				done();
			});
		});

		it('should be delete all items in the array when the data object is undefined', function (done) {
			const testItem = new List.model();
			List.fields.datearr!.updateItem(testItem, {
				datearr: ['2015-01-01', '2015-01-02', '2015-01-03', '2015-01-04'],
			}, function () {
				List.fields.datearr!.updateItem(testItem, {
					datearr: undefined,
				}, function () {
					expect(testItem.datearr).to.eql([]);
					done();
				});
			});
		});

		it('should default on null', function (done) {
			const testItem = new List.model();
			List.fields.datearr!.updateItem(testItem, {
				datearr: null,
			}, function () {
				expect(testItem.datearr).to.eql([]);
				done();
			});
		});

		it('should allow a single date value', function (done) {
			const testItem = new List.model();
			List.fields.datearr!.updateItem(testItem, {
				datearr: '2015-01-01',
			}, function () {
				expect(testItem.datearr).to.eql([new Date('2015-01-01')]);
				done();
			});
		});
	});

	describe('addFilterToQuery', function () {
		describe('"some" present', function () {
			it('should filter a specific date', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					presence: 'some',
					value: '2015-01-01',
				});
				expect(result.datearr).to.eql({
					$elemMatch: {
						$gte: dayjs('2015-01-01').startOf('day').toDate(),
						$lte: dayjs('2015-01-01').endOf('day').toDate(),
					},
				});
			});

			it('should filter after a specific datearr', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					presence: 'some',
					mode: 'after',
					value: '2015-01-01',
				});
				expect(result.datearr).to.eql({
					$elemMatch: {
						$gt: dayjs('2015-01-01').endOf('day').toDate(),
					},
				});
			});

			it('should filter before a specific datearr', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					presence: 'some',
					mode: 'before',
					value: '2015-01-01',
				});
				expect(result.datearr).to.eql({
					$elemMatch: {
						$lt: dayjs('2015-01-01').startOf('day').toDate(),
					},
				});
			});

			it('should filter between two specified datearrs', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					presence: 'some',
					mode: 'between',
					after: '2015-01-01',
					before: '2016-01-01',
				});
				expect(result.datearr).to.eql({
					$elemMatch: {
						$gte: dayjs('2015-01-01').startOf('day').toDate(),
						$lte: dayjs('2016-01-01').endOf('day').toDate(),
					},
				});
			});

			it('should not filter anything in between mode if no value is specified', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					presence: 'some',
					mode: 'between',
				});
				expect(result.datearr).to.be.undefined;
			});

			it('should not filter anything in between mode without an after datearr', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					presence: 'some',
					mode: 'between',
					before: '2015-01-01',
				});
				expect(result.datearr).to.be.undefined;
			});

			it('should not filter anything in between mode without a before datearr', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					presence: 'some',
					mode: 'between',
					after: '2015-01-01',
				});
				expect(result.datearr).to.be.undefined;
			});

			it('should not filter anything in between mode with an invalid after datearr', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					presence: 'some',
					mode: 'between',
					after: 'notadatearr',
				});
				expect(result.datearr).to.be.undefined;
			});

			it('should not filter anything in between mode with an invalid before datearr', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					presence: 'some',
					mode: 'between',
					before: 'notadatearr',
				});
				expect(result.datearr).to.be.undefined;
			});
		});

		describe('"none" present', function () {
			it('should filter a specific date', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					presence: 'none',
					value: '2015-01-01',
				});
				expect(result.datearr).to.eql({
					$not: {
						$gte: dayjs('2015-01-01').startOf('day').toDate(),
						$lte: dayjs('2015-01-01').endOf('day').toDate(),
					},
				});
			});

			it('should filter after a specific datearr', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					presence: 'none',
					mode: 'after',
					value: '2015-01-01',
				});
				expect(result.datearr).to.eql({
					$not: {
						$gt: dayjs('2015-01-01').endOf('day').toDate(),
					},
				});
			});

			it('should filter before a specific datearr', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					presence: 'none',
					mode: 'before',
					value: '2015-01-01',
				});
				expect(result.datearr).to.eql({
					$not: {
						$lt: dayjs('2015-01-01').startOf('day').toDate(),
					},
				});
			});

			it('should filter between two specified datearrs', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					presence: 'none',
					mode: 'between',
					after: '2015-01-01',
					before: '2016-01-01',
				});
				expect(result.datearr).to.eql({
					$not: {
						$gte: dayjs('2015-01-01').startOf('day').toDate(),
						$lte: dayjs('2016-01-01').endOf('day').toDate(),
					},
				});
			});

			it('should not filter anything in between mode if no value is specified', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					presence: 'none',
					mode: 'between',
				});
				expect(result.datearr).to.be.undefined;
			});

			it('should not filter anything in between mode without an after datearr', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					presence: 'none',
					mode: 'between',
					before: '2015-01-01',
				});
				expect(result.datearr).to.be.undefined;
			});

			it('should not filter anything in between mode without a before datearr', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					presence: 'none',
					mode: 'between',
					after: '2015-01-01',
				});
				expect(result.datearr).to.be.undefined;
			});

			it('should not filter anything in between mode with an invalid after datearr', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					presence: 'none',
					mode: 'between',
					after: 'notadatearr',
				});
				expect(result.datearr).to.be.undefined;
			});

			it('should not filter anything in between mode with an invalid before datearr', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					presence: 'none',
					mode: 'between',
					before: 'notadatearr',
				});
				expect(result.datearr).to.be.undefined;
			});
		});

		// Should default to "some" present behaviour
		describe('no presence option specified', function () {
			it('should filter a specific date', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					value: '2015-01-01',
				});
				expect(result.datearr).to.eql({
					$elemMatch: {
						$gte: dayjs('2015-01-01').startOf('day').toDate(),
						$lte: dayjs('2015-01-01').endOf('day').toDate(),
					},
				});
			});

			it('should filter after a specific datearr', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					mode: 'after',
					value: '2015-01-01',
				});
				expect(result.datearr).to.eql({
					$elemMatch: {
						$gt: dayjs('2015-01-01').endOf('day').toDate(),
					},
				});
			});

			it('should filter before a specific datearr', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					mode: 'before',
					value: '2015-01-01',
				});
				expect(result.datearr).to.eql({
					$elemMatch: {
						$lt: dayjs('2015-01-01').startOf('day').toDate(),
					},
				});
			});

			it('should filter between two specified datearrs', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					mode: 'between',
					after: '2015-01-01',
					before: '2016-01-01',
				});
				expect(result.datearr).to.eql({
					$elemMatch: {
						$gte: dayjs('2015-01-01').startOf('day').toDate(),
						$lte: dayjs('2016-01-01').endOf('day').toDate(),
					},
				});
			});

			it('should not filter anything in between mode if no value is specified', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					mode: 'between',
				});
				expect(result.datearr).to.be.undefined;
			});

			it('should not filter anything in between mode without an after datearr', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					mode: 'between',
					before: '2015-01-01',
				});
				expect(result.datearr).to.be.undefined;
			});

			it('should not filter anything in between mode without a before datearr', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					mode: 'between',
					after: '2015-01-01',
				});
				expect(result.datearr).to.be.undefined;
			});

			it('should not filter anything in between mode with an invalid after datearr', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					mode: 'between',
					after: 'notadatearr',
				});
				expect(result.datearr).to.be.undefined;
			});

			it('should not filter anything in between mode with an invalid before datearr', function () {
				const result = List.fields.datearr!.addFilterToQuery({
					mode: 'between',
					before: 'notadatearr',
				});
				expect(result.datearr).to.be.undefined;
			});
		});
	});
	/* Deprecated inputIsValid tests */

	it('should validate input', function () {
		expect(List.fields.datearr!.inputIsValid({
			datearr: '2015-03-03',
		})).to.be.true;
		expect(List.fields.datearr!.inputIsValid({
			datearr: ['2015-03-03'],
		})).to.be.true;
		expect(List.fields.datearr!.inputIsValid({
			datearr: ['2015-03-03', '2015-03-04'],
		})).to.be.true;
	});

	it('should validate no input', function () {
		const testItem = new List.model();
		expect(List.fields.datearr!.inputIsValid({})).to.be.true;
		expect(List.fields.datearr!.inputIsValid({}, true)).to.be.false;
		testItem.datearr = ['2015-03-03'];
		expect(List.fields.datearr!.inputIsValid({}, true, testItem)).to.be.true;
	});

	it('should validate length when required', function () {
		expect(List.fields.datearr!.inputIsValid({
			datearr: [],
		}, true)).to.be.false;
	});

	it('should invalidate arrays with invalid dates', function () {
		expect(List.fields.datearr!.inputIsValid({
			datearr: 'not a real date',
		})).to.be.false;
		expect(List.fields.datearr!.inputIsValid({
			datearr: ['2001-01-35'],
		})).to.be.false;
		expect(List.fields.datearr!.inputIsValid({
			datearr: ['35-34-3210', '2001-01-01'],
		})).to.be.false;
	});
};
