...
- **`server/startSecureServer.js`**: Starts the secure (HTTPS/SPDY) server, handling SSL certificate loading and SNI.
  - **Options**: `ssl` (boolean or 'force'), `ssl port` (number), `ssl host` (string), `ssl key` (string), `ssl cert` (string), `ssl ca` (string), `ssl pfx` (string), `ssl passphrase` (string), `ssl sni` (function)
