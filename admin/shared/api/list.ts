...
export const reorderItem = (
  ...
  api<unknown>(
    withQuery(
      `/${encodeURIComponent(listKey)}/${encodeURIComponent(id)}/sortOrder/${encodeURIComponent(String(sortOrder))}/${encodeURIComponent(String(newOrder))}`,
      params,
    ),
    { method: 'POST' },
  ).then(normalizeListResponse);
...
export function toFieldMeta(field: AdminFieldMeta): FieldMeta {
  ...
  return {
    ...
    ...(fieldType === 'relationship' ? { refList, many: field.many === true, createInline: field.createInline === true } : {}),
  } as FieldMeta;
}
...
function locationSubmissionEntries(field: AdminFieldMeta, value: unknown): Record<string, unknown> {
  if (!isRecord(value)) return { [field.path]: value };
  const { _improve, _improve_overwrite, ...location } = value;
  return {
    [field.path]: location,
    ...(_improve ? { [`${field.path}_improve`]: true } : {}),
    ...(_improve_overwrite ? { [`${field.path}_improve_overwrite`]: true } : {}),
  };
}
...
export function prepareItemData(fields: AdminFieldMeta[], data: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [path, value] of Object.entries(data)) {
    ...
    if (field?.fieldType === 'relationship') {
      ...
      Object.assign(prepared, locationSubmissionEntries(field, value));
    } else {
      prepared[path] = value;
    }
  }
