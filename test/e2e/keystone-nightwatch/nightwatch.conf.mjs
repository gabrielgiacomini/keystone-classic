...
import nightwatchJson from './nightwatch.json' with { type: 'json' };
...
export default (function (settings) {
        ...
        const driverRoot = process.env.KNE_DRIVER_ROOT || path.resolve(__dirname, '../drivers');
        let chromeDriver = resolveFromEnv(['KNE_CHROMEDRIVER_PATH', 'CHROMEDRIVER_PATH']);
        let geckoDriver = resolveFromEnv(['KNE_GECKODRIVER_PATH', 'GECKODRIVER_PATH']);

        if (!chromeDriver) {
                const chromeBinary = isWindows ? 'chromedriver.exe' : 'chromedriver';
                const chromePlatform = inferChromePlatform(isWindows);
        ...
        if (!geckoDriver) {
                try {
                        ...
                        const geckoDriverPath = fileURLToPath(import.meta.resolve('geckodriver'));
        ...
        settings.globals_path = path.resolve(__dirname, 'globals.mjs');
        if (chromeDriver) {
