			...
			if (config) {
				if (config.fields) {
					config.fields.forEach(function (field) {
						const fieldSelector = getItemFieldSelector(field);
						if (fieldSelector) {
							const ModelTestConfig = field.modelTestConfig || browser.defaultModelTestConfig;
							if (ModelTestConfig) {
								const modelTestConfig = new ModelTestConfig({ formSelector: fieldSelector });
								if (modelTestConfig) {
									const fieldTestObject = modelTestConfig[field.name];
									if (fieldTestObject) {
