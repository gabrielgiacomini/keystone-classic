 ...
 * @private
 */
export default function HomeScreenTab (config) {
	const selectElem = function (elem) {
		return self.selector + ' ' + self.elements[elem];
	};

	const listPath = utils.keyToPath(config.listName, true);
	const listLabel = utils.keyToLabel(listPath);
	...
	var self = {
		...
		commands: {
			...
			clickTabUI: function (browser, clickable, options) {
				...
				const wait = options && options.wait || true;
				if (wait) {
