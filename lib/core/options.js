/**
 * @fileoverview This file contains methods for setting and getting Keystone's options.
 * It is responsible for handling and validating all the configuration options that can be passed to Keystone.
 * @module lib/core/options
 */
	...
	switch (key) {
		...
		case 'frame guard':
			...
			if (typeof value === 'string') {
				...
				if (validFrameGuardOptions.indexOf(value) < 0 && value.substring(0, 11) !== 'allow-from ') {
					value = false;
				}
 ...
 * @param {string} [defaultValue] The default value to return if the option is not set.
 * @returns {string} The expanded path.
 * @example
 * keystone.getPath('pathOption', 'defaultValue');
 */
